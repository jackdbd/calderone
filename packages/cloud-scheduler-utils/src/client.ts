...
/**
 * Initializes the Cloud Scheduler client from the environment.
 */
export const cloudScheduler = (options: Options = DEFAULT_OPTIONS) => {
  ...
  if (client_options) {
    ...
    return new CloudSchedulerClient({ credentials, projectId })
  } else {
    details.push(`process.env["${env_key}"] not set`)
  }

  const summary = 'Cannot initialize Cloud Scheduler in this environment'
